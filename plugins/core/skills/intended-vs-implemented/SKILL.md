...
1. **Establish intent.** Read the system docs (the host project's docs convention, e.g. a project's `02-architecture/` folder, falling back to `documentation/`) as the source of truth for what *should* be true: who may access what, which boundaries are trusted, which data is public. Treat the docs as claims to verify, not as proof.

2. **Gather implementation evidence.** Read the code that enforces (or fails to enforce) each claim. Evidence is a cited file and line — the actual authorisation check, the actual query filter, the actual sanitiser. "It's probably handled upstream" is not evidence; the code path is.
